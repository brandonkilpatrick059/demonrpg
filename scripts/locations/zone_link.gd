...
@export var state_check : String = ""
@export var state_check_val : String = ""
...
func _on_body_entered(body: Node2D) -> void:
	if(body.is_in_group("player")):
		var can_enter = true
		if(state_check != ""):
			var game_state : GlobalGamestate = get_tree().get_first_node_in_group("gamestate")
			if(game_state.get_state_map_value(state_check) != state_check_val):
				can_enter = false
		if(can_enter):
